            ...
            <div class="overflow-x-auto">
                <table class="table table-sm table-pin-rows table-pin-cols">
                    ...
                    <tbody id="contenido-productos">
                        ...
                        <td>
                            <input type="number" min="1" value="{{$producto->pivot->cantidad}}"
                                   class="editable-cantidad w-full px-2 py-1 border rounded">
                        </td>

                        <td>
                            <textarea class="editable-instrucciones w-full px-2 py-1 border rounded">{{$producto->pivot->instrucciones ?? ''}}</textarea>
                        </td>
...
<script>
    ...
    // Agregar estas funciones al final del script existente

// Función para actualizar los valores ocultos desde los campos editables
function actualizarValoresOcultos() {
    $('#contenido-productos tr').each(function() {
        const productoId = $(this).data('producto-id');
        const cantidad = $(this).find('.editable-cantidad').val();
        const instrucciones = $(this).find('.editable-instrucciones').val();

        $(this).find('input[name="producto['+productoId+'][cantidad]"]').val(cantidad);
        $(this).find('input[name="producto['+productoId+'][instrucciones]"]').val(instrucciones);
    });
}

// Actualizar valores ocultos antes de enviar el formulario
$('form').on('submit', function(e) {
    actualizarValoresOcultos();

    // Opcional: Validar que haya al menos un producto
    if ($('#contenido-productos tr').length === 0) {
        e.preventDefault();
        Swal.fire('Error', 'Debe agregar al menos un producto', 'error');
    }
});

// También actualizar al cambiar los valores (opcional)
$(document).on('change', '.editable-cantidad, .editable-instrucciones', function() {
    const tr = $(this).closest('tr');
    const productoId = tr.data('producto-id');
    const cantidad = tr.find('.editable-cantidad').val();
    const instrucciones = tr.find('.editable-instrucciones').val();

    tr.find('input[name="producto['+productoId+'][cantidad]"]').val(cantidad);
    tr.find('input[name="producto['+productoId+'][instrucciones]"]').val(instrucciones);
});
